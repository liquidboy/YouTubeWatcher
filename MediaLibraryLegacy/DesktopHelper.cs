...
namespace MediaLibraryLegacy
{
    ...
    {
        public static void SendWindowToBackground(string windowTitleStartsWith) {
            var foundWindows = AppWindowEx.GetWindows();

            var foundLegacyWindows = foundWindows.Where(x => x.Title.StartsWith(windowTitleStartsWith, StringComparison.InvariantCultureIgnoreCase));
